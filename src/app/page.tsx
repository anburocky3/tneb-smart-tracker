...
export default function MultiPropertyDashboard() {
  ...
  const [sortBy, setSortBy] = useState<
    "name" | "amount_desc" | "units_desc" | "status" | "custom"
  >("custom");
  ...
  useEffect(() => {
    const savedState = localStorage.getItem("tneb_group_states");
    if (savedState) {
      try {
        setExpandedGroups(JSON.parse(savedState));
      } catch (e) {
        console.error("Failed to parse group states", e);
      }
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("tneb_group_states", JSON.stringify(expandedGroups));
  }, [expandedGroups]);
  ...
  const fetchSingleSummary = async (
    ...
    try {
      ...
      if (result.success) {
        ...
        if (forceRefresh) showToast(`Updated ${conn.nickname}`);
  ...
  const toggleGroup = (loc: string) => {
    setExpandedGroups((prev) => ({ ...prev, [loc]: !prev[loc] }));
  };

  const toggleAllGroups = () => {
    const allExpanded = Object.values(expandedGroups).every(
      (val) => val === true,
    );
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (type === "LOCATION") {
      const newLocationOrder = [...availableLocations];
      const [removed] = newLocationOrder.splice(source.index, 1);
      newLocationOrder.splice(destination.index, 0, removed);

      try {
        await fetch("/api/connections", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ locationOrder: newLocationOrder }),
        });
        const res = await fetch("/api/connections");
        const data = await res.json();
        if (Array.isArray(data)) setConnections(data);
      } catch (e) {
        console.error("Failed to save location order", e);
      }
    } else if (type === "METER") {
      const sourceLoc = source.droppableId;
      const destLoc = destination.droppableId;

      const sourceMeters = connections.filter(
        (c) => (c.location || "Unassigned Location") === sourceLoc,
      );
      const movedItem = sourceMeters[source.index];
      if (!movedItem) return;

      const remainingConnections = connections.filter(
        (c) => c.consumerNo !== movedItem.consumerNo,
      );

      const updatedMovedItem = { ...movedItem, location: destLoc };

      const destMeters = remainingConnections.filter(
        (c) => (c.location || "Unassigned Location") === destLoc,
      );

      let insertIndex: number;
      if (destination.index < destMeters.length) {
        const targetConsumerNo = destMeters[destination.index].consumerNo;
        insertIndex = remainingConnections.findIndex(
          (c) => c.consumerNo === targetConsumerNo,
        );
      } else if (destMeters.length > 0) {
        const lastConsumerNo = destMeters[destMeters.length - 1].consumerNo;
        insertIndex =
          remainingConnections.findIndex(
            (c) => c.consumerNo === lastConsumerNo,
          ) + 1;
      } else {
        insertIndex = remainingConnections.length;
      }

      const newConnections = [...remainingConnections];
      newConnections.splice(insertIndex, 0, updatedMovedItem);
      ...
      try {
        await fetch("/api/connections", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            orderedConsumerNos: newConnections.map((c) => c.consumerNo),
            updatedConnection: {
              consumerNo: updatedMovedItem.consumerNo,
              location: destLoc,
            },
          }),
        });
      } catch (e) {
        console.error("Failed to save meter order", e);
      }
    }
  };
  ...
  return (
    <div className="flex h-dvh bg-[#f8fafc] font-sans text-slate-900 overflow-hidden">
      ...
      >
                ...
                <>
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <button
                      ...
                      className="hidden md:flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-medium shadow-sm hover:bg-slate-50 transition shrink-0"
                    >
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-10 mt-6 space-y-8">
          ...
          <DragDropContext onDragEnd={onDragEnd}>
            <Droppable droppableId="all-locations-droppable" type="LOCATION">
              {(provided) => (
                <div
                ...
                >
                  {Object.entries(groupedByLocation).map(
                    ([locName, meterList]) => {
                      const isExpanded = expandedGroups[locName] !== false;
                      const totalUnits = meterList.reduce(
                        (acc, c) =>
                          acc +
                          (dashboardSummaries[c.consumerNo]?.bills?.[0]
                            ?.units || 0),
                        0,
                      );
                      ...
                      const colorClass = getLocationColor(locName);

                      return (
                        <Draggable
                          key={locName}
                          draggableId={locName}
                          index={availableLocations.indexOf(locName)}
                        >
                          {(provided) => (
                            <section
                              ref={provided.innerRef}
                            ...
                            >
                              <div
                                onClick={() => toggleGroup(locName)}
                                className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-3 cursor-pointer hover:bg-slate-50 transition-colors p-1 rounded-t-xl group"
                              >
                                <div className="flex items-center gap-2">
                                  <div
                                    {...provided.dragHandleProps}
                                    className="p-1 text-slate-400 hover:text-slate-600 transition-colors cursor-grab active:cursor-grabbing shrink-0"
                                  >
                                  ...
                                  <div
                                    className={`p-1.5 rounded-lg ${colorClass}`}
                                  >
                              ...
                              {isExpanded && (
                                <Droppable droppableId={locName} type="METER">
                                  {(provided) => (
                                    <div
                                      ref={provided.innerRef}
                                      {...provided.droppableProps}
                                          ...
                                          >
                                            {(provided) => (
                                              ...
                                              >
                                                <div
                                                  {...provided.dragHandleProps}
                                                  className="absolute top-4 left-4 p-1 text-slate-400 hover:text-slate-600 transition-colors cursor-grab active:cursor-grabbing opacity-100 md:opacity-0 md:group-hover:opacity-100"
                                                >
                                                  <GripVertical className="w-4 h-4" />
                                                </div>
                                                ...
                                                <div className="flex items-start gap-3.5 mb-5 pr-16 md:pr-0 pl-8 md:pl-0">
                                                  <div
                                                    className={`p-2.5 md:p-3 rounded-xl md:rounded-2xl shrink-0 ${colorClass}`}
                                                  >
      ...
      {toastMsg && (
        <div
          className={`fixed bottom-28 md:bottom-6 right-4 md:right-6 z-50 bg-slate-900/95 backdrop-blur text-white px-4 md:px-5 py-3 md:py-3.5 rounded-2xl shadow-2xl flex items-center gap-3 transition-all duration-500 ease-out transform max-w-[90vw] md:max-w-sm ${isToastVisible ? "translate-y-0 opacity-100 scale-100" : "translate-y-8 opacity-0 scale-95 pointer-events-none"}`}
        >
